...
<Image src="https://files.readme.io/057375b-Deals_Organizations__Persons_Labels.png" alt="3126" caption="Labels for the Deal, Organization, and Person in the Deal's details view" />

<Image src="https://files.readme.io/d00b2e4-Lead_Labels.png" alt="3218" caption="Label for a Lead displayed in the Lead's details view" />
...
| • Lead • (Updating a label) | [`PATCH /leadLabels/{id}`](https://developers.pipedrive.com/docs/api/v1/LeadLabels#updateLeadLabel)                       | **id** (string) - label field ID         |
| • Lead • (Adding a label)   | [`POST /leadLabels`](https://developers.pipedrive.com/docs/api/v1/LeadLabels#addLeadLabel)                                | • **name** (string) • **color** (string) |
...
**Sample request body** We added a Lead label with the name `Tentative` in the color `gray`.

<CodeGroup tabs="JSON,node.js">
  ```json title="JSON"
  {
     "name":"Tentative",
     "color":"gray"
  ...
  // Configure authorization using api key
  // PIPEDRIVE_API_KEY is an environment variable that holds real api key
  const config = new pipedrive.Configuration({ apiKey: process.env.PIPEDRIVE_API_KEY });

  async function addLeadLabel() {
      try {
          console.log('Sending request...');

          const api = new pipedrive.LeadLabelsApi(config);

          const data = {
              name: 'Tentative',
              color: 'gray'
          }
          const response = await api.addLeadLabel({ AddLeadLabelRequest: data });

          console.log('Lead label added successfully!', response);
      } catch (err) {
          const errorToLog = err.response?.data || err;
...
**Request body** The title is the only required field here. The `person_id` can also be replaced by the `organization_id`.
...
<CodeGroup tabs="JSON,node.js">
  ...
  async function addLabel(name, color) {
      ...
      const label = await api.addLeadLabel({ AddLeadLabelRequest: { name, color } });
  ...
  async function addLead() {
      try {
          ...
          const api = new pipedrive.LeadsApi(config);
          const redLabel = await addLabel('Custom Label 1', 'red');
          const greenlabel = await addLabel('Custom Label 2', 'green');
          const data = {
              title: 'Superman lead',
              person_id: 1,
              // label_ids takes an array of label ids that you can create with addLeadLabelApi
              label_ids: [
                  redLabel,
                  greenlabel
              ]
...
<CodeGroup tabs="JSON,node.js">
  ...
  async function addLabelToLead() {
      try {
          ...
          const response = await api.updateLead({ id: LEAD_ID, UpdateLeadRequest: data });
