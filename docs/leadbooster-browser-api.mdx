...
<Callout kind="info">
  Before using the LeadBooster browser API, make sure you’re using the latest LeadBooster embed script. We recommend updating the embed script before using this API. You can find the latest script in the Pipedrive web app by going to _Leads > LeadBooster > Live Chat > select the Playbook > Installation > Manual installation_.
</Callout>
