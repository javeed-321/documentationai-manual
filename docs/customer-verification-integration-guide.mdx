---
title: "Customer Verification: Integration Guide"
---
...
## Step 1: [Create an Application](/reference/createapplication)
...
Skip this step if you are using the SDK-only route. Jump straight to [Step 5](/docs/customer?isFramePreview=true#step-5-create-a-short-lived-token).
...
### [Create a Compliance Associate](/reference/createapplicationassociate)
...
### [Submit KYB/KYC Data](/reference/updateknowyourcustomerbyapplicationid)
...
### [Submit Associate Personal Info](/reference/updatepersonalinformationforassociate_1)
...
### [Submit Associate Tax Residencies](/reference/updatetaxresidenciesforassociate_1)
...
See the full webhook reference: [Webhook - Application Status Change](/docs/webhook-application-status-change)
...
## Step 4 (optional): [Upload ID verification docs / biometric selfie image](/reference/uploaddocument)
...
## Step 5: [Initiate Application Verification](/reference/verifyapplication)
...
## Step 6: [Create a Short-lived Token](/reference/createsdkkey)
...
2. Re-invoke the SDK (repeat [Steps 6-9](/docs/customer?isFramePreview=true#step-5-create-a-short-lived-token)) using the same `applicationId`. A fresh short-lived token is required for each invocation.
...
Store the `customerId` against your customer record. Use it in subsequent [Account Creation API](/reference/createaccount) calls.

See the full webhook reference: [Webhook - Customer Created](/docs/webhook-customer-created)
...
See the full webhook reference: [Webhook - Customer Status Change](/docs/customer-status)
