---
title: "Pipedrive API v2 migration guide"
---

---

## updatedAt: 2026-04-30T07:24:34.000Z
...
- [Project Boards API](#project-boards-api)
  - [Project Board Object](#project-board-object)
- [Project Phases API](#project-phases-api)
  - [Project Phase Object](#project-phase-object)
- [Project Templates API](#project-templates-api)
  - [Project Template Object](#project-template-object)
- [Project Tasks API](#project-tasks-api)
  - [Project Task Object](#project-task-object)
...
- `product`, \`\`quantity\_formatted`and`sum\_formatted\` fields are no longer included.
...
```json
{
  ...
  "type": "call",
  "is_deleted": false, // Replaces `active_flag`. NB: Negation of old value.
  "done": false,
  "conference_meeting_client": null,
  "conference_meeting_url": null,
  "conference_meeting_id": null,
...
_Returned field object has changed, see [Field Object](#field-object) for details._
...
- `person_id` field has been replaced with `person_ids`. For now only linking to a single person is supported.
- `org_id` field has been replaced with `org_ids`. For now only linking to a single person is supported.
- `labels` field has been renamed to `label_ids`.

#### Old v1 Project Object Example
...
```json
{
  ...
  "description": "Description",
  "status": "open",
  "status_change_time": "2026-04-14 10:45:20.852", // now in TZ format
  "start_date": "2026-04-15",
  "end_date": "2026-04-23",
  "owner_id": 1,
  "add_time": "2026-04-14 10:45:20.852", // now in TZ format
  "update_time": "2026-04-14 10:45:20.852", // now in TZ format
  "archive_time": null, // now in TZ format
  "labels": [1], // Renamed to `labels`
  "deal_ids": [1],
  "person_id": 1, // Replaced with `person_ids` array
  "org_id": 1, // Replaced with `org_ids` array
  "board_id": 1,
  "phase_id": 1,
  "health_status": null
	"53c2f18db6a1655d6af8bba77d9679565f975fd8": "Text Custom Field", // wrapped in custom_fields object
...
```json
{
  ...
  "description": "Description",
  "status": "open",
  "status_change_time": "2026-04-14T10:45:20.852Z", // now in TZ format
  "start_date": "2026-04-15",
  "end_date": "2026-04-23",
  "owner_id": 1,
  "add_time": "2026-04-14T10:45:20.852Z", // now in TZ format
  "update_time": "2026-04-14T10:45:20.852Z", // now in TZ format
  "archive_time": null, // now in TZ format
  "label_ids": [1], // Renamed from `labels`
  "board_id": 1,
  "phase_id": 1,
  "deal_ids": [1],
  "person_ids": [1], // Now an array
  "org_ids": [1], // Now an array
  "health_status": null
  "custom_fields": { // custom fields are no longer flat root level fields
      "53c2f18db6a1655d6af8bba77d9679565f975fd8": "Text Custom Field",
      "d4de1c1518b4531717c676029a45911c340390a6": {
...
### GET /api/v1/projects to GET /api/v2/projects and GET /api/v2/projects/archived

_Please note that this section does not include the changes already covered in [General API Changes](#general-api-changes) section._

- `include_archived` parameter has been removed. Please use separate GET /api/v2/projects/archived endpoint to fetch archived projects.
...
- `add_time` and `update_time` are now in TZ format.

#### Old v1 Board Object Example
...
- `add_time` and `update_time` are now in TZ format.
- The endpoints are now prefixed with just `/api/v2/boards` instead of `/api/v1/projects/boards`
...
- `add_time` and `update_time` are now in TZ format.
- The endpoints are now prefixed with just `/api/v2/phases` instead of `/api/v1/projects/phases`

#### Old v1 Phase Object Example
...
- `add_time` and `update_time` are now in TZ format, no other changes.
...
- `assignee_id` field has been replaced with `assignee_ids`.
- `done` field has been renamed to `is_done` and converted to a boolean.
- `milestone` field has been renamed to `is_milestone`and converted to a boolean.
- `priority` numeric value has been replaced with an object `{ id: number }`.

#### Old v1 Task Object Example
...
```json
{
  ...
  "project_id": 1
  "done": 1, // Renamed to is_done and is a boolean now
  "milestone": 0, // Renamed to is_milestone and is a boolean now
  "due_date": "2024-11-04",
  "start_date": null,
  "parent_task_id": null,
  "assignee_id": 6192726, // Removed in favour of `assignee_ids`
  "assignee_ids": [6192726],
  "add_time": "2024-11-04 08:47:01.235", // in TZ format now
  "update_time": "2026-03-14 10:38:24.027", // in TZ format now
  "marked_as_done_time": "2024-11-04 08:47:01.233", // in TZ format now
  "priority": 26, // Converted to { id: number } object
}
```

#### New v2 Task Object Example
...
```json
{
  ...
  "description": "Task description",
  "project_id": 1,
  "is_done": true, // Renamed from 'done' and is a boolean now
  "is_milestone": false, // Renamed from 'milestone' and is a boolean now
  "due_date": "2024-11-04",
  "start_date": null,
  "parent_task_id": null,
  "assignee_ids": [6192726],
  "add_time": "2024-11-04T08:47:01.235Z", // in TZ format now
  "update_time": "2026-03-14T10:38:24.027Z", // in TZ format now
  "marked_as_done_time": "2024-11-04T08:47:01.233Z", // in TZ format now 
  "priority": { id: 26 } // Previously just plain ID
}
```
