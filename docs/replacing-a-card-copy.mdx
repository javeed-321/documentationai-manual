---
title: "Replacing a card"
---

In the event that something has happened to your card and you would like a new one to replace it, the [Replace a card](/reference/replacecard) can be used.
...
<Callout kind="info">
  **If the card being replaced is a PHYSICAL card then there is a requirement to use the [Active Physical Card ](/reference/activatecard) endpoint once the card has been activated.**
...
See [Getting started with Physical Cards](/docs/physical-cards-overview) for detail.
...
If the PIN is required to be changed then use the [Reset card PIN](/reference/resetpin) end point after the [Replace a card](/reference/replacecard) end point has been completed.
