...
1. **Modulr developer credentials** — see [Getting Started](/docs/getting-started) to begin your orientation and [Getting Sandbox Access](/docs/gaining-use-of-the-api) to get your sandbox base URL, API token, and account ID. You'll need these once you start making real API calls.
2. **Authentication configured** — see [Authentication](/docs/authentication). Use HMAC authentication from the start — it is mandatory in production
...
The MCP works best alongside the existing documentation. If you're new to Modulr, start with the [Fundamentals](/docs/fundamentals) section first — the MCP accelerates development, but it doesn't replace understanding how the platform works.
...
- **Email:** [implementation@modulrfinance.com](mailto:implementation@modulrfinance.com)
