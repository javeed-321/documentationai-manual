...
To submit a payment batch via the API, use [Make a batch payment](/reference/submitbatchpayments).

The request must at a minimum contain an array of payments. The format of these is the same as [Create a payment](/reference/sendpayment).
...
Use [Get batch payments by a given set of parameters](/reference/getbatchpayments) and query using parameters, such as an _externalReference_ that you might have provided for the batch, to retrieve the batch's _batchId_ value.

Then, use [Cancel the batch payment](/reference/cancelbatchpayments) with the _batchId_ value, to cancel all applicable payments\
...
by assigning the smaller batches with the same _externalReference_ string value of your own choosing, to\
manage them as a group later on.

To cancel all the payments within the original group of batch payments, use [Get batch payments by a given set of parameters](/reference/getbatchpayments) and query using the _externalReference_ that was provided to the group of batch payments, and receive all the relevant _batchId_'s

For each of the _batchId_ obtained in Step 1,  use the [Cancel the batch payment](/reference/cancelbatchpayments) to cancel all applicable payments within the batch
