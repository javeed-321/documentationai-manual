...
- **Call options** allow the holder to _purchase_ the underlying at the strike price, on or before the expiration date
- **Put options** allow the holder to _sell_ the underlying at the strike price, on or before the expiration date
...
Once the [automated option approval process ](https://developer.drivewealth.com/apis/v1.83.0/docs/enabling-options-features)has been completed for a user and they’ve been approved for options trading, their account can be enabled for options trading.

## Step 3: Discovering an option contract to trade

<Image src="https://files.readme.io/65f8011-Discover-Bull2x.png" alt="Discover Bull2x" caption=" " />
...
Learn more and review specific implementation details in [Showing options chains](/docs/filtering-options).
...
DriveWealth clients can leverage the [Orders API](https://developer.drivewealth.com/apis/reference/post_orders)  used for other asset classes to submit options orders. Additional order validations exist to ensure that appropriate options attributes are included in the order submission.

| Order Type |         Order Mode        | TIF | Slides Supported           |
| :--------- | :-----------------------: | --: | :------------------------- |
| Limit      | Whole Contract (quantity) | Day | • BUY\_OPEN, • SELL\_CLOSE |

Please refer to [Creating an Order](https://developer.drivewealth.com/apis/docs/creating-an-order)  for implementation details and an example of placing an options order.
...
Please refer to [Getting balances and positions](/docs/getting-balances-and-positions) for more details on reviewing an account’s position summary, inclusive of options positions
...
Contract holders may also submit a “Do Not Exercise” request to prevent an ITM position from being exercised by The OCC on expiration day.  For more information see [exercising contracts and managing expiration](https://developer.drivewealth.com/apis/v1.83.0/docs/exercising-contracts-managing-expiration).
