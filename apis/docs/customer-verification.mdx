...
<Image src="https://files.readme.io/feed41f-6a18b92-KYC_Status.png" alt="6a18b92-KYC_Status.png" caption="At any point beyond `KYC_PROCESSING`, the User's KYC can be approved or denied." />
...
In sandbox, you can test error cases by setting the `lastName` of a User to the specific KYC error you want to simulate, with `_` replaced by a space. For instance, `DOB_NOT_MATCH` can be entered as `DOB NOT MATCH`.
