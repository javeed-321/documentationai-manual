...
_Two&#x20;_`instruments.updated`_events (expiring the pre-existing symbol, and setting the new symbol as active)_ Two `positions.updated` (removal of the position in pre-existing symbol and adding a position in the new one). \* Transactions will reflect the change as well
...
Transactions: These transactions are available through List Account Transactions [https://developer.drivewealth.com/apis/reference/get\_accounts-accountid-transactions](https://developer.drivewealth.com/apis/reference/get_accounts-accountid-transactions)
