---
updatedAt: 2026-07-24T14:44:24.000Z
---
...
For details on all the parameters needed for the POST /mandates/id/collectionschedules endpoint the documentation is here: [Create a scheduled collection](https://modulr.readme.io/reference/createcollectionschedule)
...
<Image src="https://files.readme.io/96ce028831c5149fedc9aca3bbf9c57325f8679bc4a0838c87fee8c67967efaa-DD_Collections_cycle_at_Modulr.drawio.png" align="center" />

**Existing mandate and new collection**<br />For existing mandates and collection schedules, the Partner can be assured that Modulr will create and send the collection request two working days before the collection date. The Partner does not have to do anything else and the timeline is shorten because of an existing mandate on a customer's account.

![](https://files.readme.io/801f143-DD_Collection_Process.png "DD Collection Process.png")

## Timeline of events for Collections

*Note: Timings are current UK Timezone GMT/BST as appropriate*
...
2. Modulr sends the [Webhook - Upcoming Collection Credit](https://modulr.readme.io/docs/webhook-direct-debit-ddcollectionstatus) at 4:45PM to tell the Partners that funds will arrive in their account tomorrow.

Day 3:<br />The funds are debited from the Paying bank regardless if the customer has any sufficient funds or not.
...
2. When Modulr credits the funds into the Partner's account, we send the [PayIn Webhook](https://modulr.readme.io/docs/payin-webhook) to notify them.
...
![](https://files.readme.io/3ceaced-New_Mandate_and_Collection-New_Collection_-_Happy_3.jpg "New Mandate and Collection-New Collection - Happy (3).jpg")

**Unhappy Path**<br />ARUDDs are messages to tell Modulr that a collection was unsuccessful due to a variety of reasons (i.e. Insufficient funds, no DDI, and more). It will trigger the process of returning funds to the Paying bank on Day 4 and Day 5.

Day 3:

1. Modulr receives the funds from the Paying Bank and transfers the funds into the Partner's account at 03:00 AM to 07:00AM.
2. When Modulr credits the funds into the Partner's account, we send the [Webhook - Inbound Payments](https://modulr.readme.io/docs/payin-webhook)  to notify them.
...
2. Modulr sends [Webhook - Direct Debit, DD Collection Status](https://modulr.readme.io/docs/webhook-direct-debit-ddcollectionstatus) to our Partner informing them of the failed collection at 10:15 AM.

Day 5:

1. Modulr debits the Partner's account and returns the amount to the Paying bank. The collection’s status is “Failed”.

**Late Unhappy Collections**<br />In the vast majority of cases, the Paying Bank sends an ARUDD on Day 3. However, in a minority of cases they send the ARUDD on Day 4. The process will be similar to the unhappy collection but will start at **Day 4** where the Paying bank generates the file and sends to the scheme and so on.

![](https://files.readme.io/ba28de0-New_Mandate_and_Collection-New_Collection_-_Unhappy_Path_4.jpg "New Mandate and Collection-New Collection - Unhappy Path (4).jpg")

**Updates on Collection progress**<br />Modulr provides a [GET /collections endpoint](https://modulr.readme.io/reference/getcollections) which will give the Partner a list of collection activities related to an account, these will account for collections and any indemnities.
...
![](https://files.readme.io/245c0f5-collection_api_flow.png "collection api flow.png")

**Create Collection Schedule API** - <https://modulr.readme.io/reference/createcollectionschedule><br />**Get All Collection Schedule API**  - <https://modulr.readme.io/reference/getcollectionschedules><br />**Get All Collection Activities of an account API** - <https://modulr.readme.io/reference/getcollections><br />**Cancel Collection Request API** - <https://modulr.readme.io/reference/cancelcollection>

**PAYIN Webhook** - <https://modulr.readme.io/docs/payin-webhook><br />**UPCOMINGDDCOLLECTIONCREDIT Webhook** - <https://modulr.readme.io/docs/upcomingddcollectioncredit-webhook><br />**DDCOLLECTIONSTATUS** -  <https://modulr.readme.io/docs/webhook-direct-debit-ddcollectionstatus>

## Representment of unpaid Direct Debits

A Direct Debit that has been returned unpaid with a status of **Representable** may be retried using the [represent collection ](https://modulr.readme.io/reference/representcollection)endpoint. There are specific scheme rules about the ability to represent and process in the Service Users Guide and Rules to the Direct Debit Scheme that must be followed - see extract below:

*Re-presentation of an unpaid Direct Debit may continue for****one month****&#x20;from the date on which first presentation was made.*

*Rule: If payment has not been made within this period, the service user must make other arrangements directly with the payer for collecting the amount due or give advance notice of the new collection date and amount. Refer to section 4B ‘Advance notice’.f the Service Users Guide and Rules to the Direct Debit Scheme]\_&#xA;*

*Guidance:*

* *If a Direct Debit is returned unpaid by the paying PSP it is recommended that the service user should liaise with the payer to offer a new collection date. A service user should give at least 5 working days’ notice to the payer of the new collection date*
* *If the Direct Debit has been returned for the reason ‘refer to payer’ then additional advance notice is not mandatory but is recommended.*
...
**Creating your first mandate on the Sandbox**<br />Once set up, the first logical step is to try to use the POST /accounts/\{id}/mandates endpoint to [create a mandate](https://modulr.readme.io/reference/createmandate). Typically, when you have built your user interface, your app will be using this endpoint to send through the information to Modulr that was entered by your customers in your mandate form.
...
**Things to note:**<br />The dates in the call should be tomorrow's date, or date for which you will attempt to GET collections. We process mocked collections on daily schedule. I.e. if you create a mock call today, you will be able to retrieve the collections through the GET call tomorrow.
...
<Callout icon="📘" theme="info">
  ...
  If there is a collection failure due to any reason (such as insufficient funds), you will know the next working day via [Webhook - Direct Debit, DD Collection Status](https://modulr.readme.io/docs/webhook-direct-debit-ddcollectionstatus) after the funds has been credited to your account.
